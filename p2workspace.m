...
for i = 1:1:601
    ...
    for j = 1:1:501
        ...
        B1Bsqure = (xe(i) - rplat*sin(alpha*pi/180))^2 + (ye(j) + rplat*cos(alpha*pi/180) - rbase)^2;
...
xlabel('x/mm'); 
ylabel('y/mm');
s = strcat('PPR workspace with alpha =', {' '}, int2str(alpha));
title(s)
axis equal
axis([-400 400 -250 350])
text(-rbase/2*sqrt(3) - 20, -rbase/2 - 20, 'PB1') ;
text(rbase/2*sqrt(3) + 10, -rbase/2 - 10, 'PB3') ;
text(0, rbase + 20, 'PB2') ;
